...
/* ---------- the Brain: GALACTIC-CORE nebula + knowledge web + swirling outer band ----------
   M4 "HERO BRAIN" (design call: A+D from the reference video — a dark galaxy-core map where the
...
export function makeNeuralBrain() {
  ...
  bandG.add(band);
  /* links to the outer nodes (M5.5 — the band dots floated unconnected): local
  ...
  /* M5: the M4 "space orb" dark backing is DELETED — on the cream office it read as a
     storm-cloud shadow. The cream palette above needs no backing; galaxy mode (gk→1)
